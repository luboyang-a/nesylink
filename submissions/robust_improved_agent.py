...
class Policy:
    ...
    def act(self, obs: np.ndarray, info: dict[str, Any] | None = None) -> int:
        ...
        if adjacent_monsters and "sword" in tools_now and not scripted_task4:
    ...
    def _commit_script_room_change(self) -> None:
        ...
        if self.task_id.endswith("task_5") and self.task5_pending_exit is not None:
            transitions = {
                ("hub", "south"): "south",
                ("south", "north"): "hub",
                ("hub", "east"): "east",
            ...
            self.task5_logic_room = transitions.get(
                (self.task5_logic_room, self.task5_pending_exit),
    ...
    def _task5_script_target(
        ...
        if self.seen_events.get("monster_killed", 0) <= 0:
            return ("force_monster", (7, 4))
    ...
    def _script_allowed_tiles(self) -> set[Position] | None:
        ...
        if self.task_id.endswith("task_5"):
            # The hub has a few fixed walls that matter for the route from the
            # first chest to the monster/button. The branch rooms are simple
            # enough that the fixed goals are reachable without additional
            # hidden-state knowledge.
            if self.task5_logic_room == "hub":
                return all_tiles - TASK5_HUB_WALLS
            return all_tiles
