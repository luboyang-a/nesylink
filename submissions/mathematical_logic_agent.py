...
def _room_hint(
    ...
    if (7, 1) in chests or ((2, 2) in walls and (2, 3) in walls and exits["west"]):
        return "task5_east"

    if (2, 6) in chests or ((1, 2) in walls and (5, 5) in walls):
        return "task5_west"
...
class Policy:
    ...
    def _select_target(
        ...
        if self.task_id.endswith("task_2"):
            if scene.monsters:
                return ("monster", min(scene.monsters, key=lambda p: manhattan(scene.player, p)))
            if keys <= 0:
                return self._nearest_chest(scene)
            return ("exit", "west")
        ...
        if self.task_id.endswith("task_5"):
            return self._task5_target(scene, keys)
    ...
    def _nearest_chest(self, scene: Scene) -> tuple[str, Position] | None:
        unopened = [
            ...
            if (scene.room_hint, chest) not in self.opened
        ]

        if not unopened:
            return None

        return ("open", min(unopened, key=lambda p: manhattan(scene.player, p)))
    ...
    def _task5_target(self, scene: Scene, keys: int) -> tuple[str, Any] | None:
        if scene.monsters and min(manhattan(scene.player, m) for m in scene.monsters) <= 2:
            return ("monster", min(scene.monsters, key=lambda p: manhattan(scene.player, p)))
        ...
        if scene.buttons:
            button = min(scene.buttons, key=lambda p: manhattan(scene.player, p))
            if scene.room_hint == "task5_start" and "task5_south" not in self.visited_hints:
                return ("walk", button)

        if scene.room_hint == "task5_south":
            return ("exit", "north")

        if scene.room_hint == "task5_east":
            return ("exit", "west")

        if scene.room_hint == "task5_west":
            return None

        if keys > 0 and "task5_east" not in self.visited_hints:
            return ("exit", "east")

        if "task5_south" not in self.visited_hints:
            return ("exit", "south")

        if "task5_west" not in self.visited_hints:
            return ("exit", "west")
