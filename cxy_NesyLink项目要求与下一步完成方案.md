...
| 文件 | 用途 | 修改规则 |
|---|---|---|
| `submissions/robust_final_agent.py` | 我的最终提交主入口 | 日常修改和测试使用这个文件；会导入下面两个固定辅助策略 |
| `submissions/robust_final_legacy_agent.py` | 最终版兼容策略 | 保存我 7 月 14 日 04:51 版本中已验证的实现，一般不直接修改 |
| `submissions/robust_final_snapshot_agent.py` | 最终版基线快照 | 固定保存已验证的团队基线分支，不随公共 Agent 变化 |
...
| 工作项 | 当前状态 | 说明 |
|---|---|---|
| 个人最终 Agent 主体 | P0 已实现 | `robust_final_agent.py` 已有像素抽取、符号地图、BFS、房间记忆和任务高层逻辑 |
...
```text
submissions/robust_final_agent.py
```

最终提交采用三文件结构：`robust_final_agent.py` 是唯一主入口，依赖固定的 `robust_final_legacy_agent.py` 和 `robust_final_snapshot_agent.py`。三者共同对应已经完成正式评测的原三文件策略，仅整理命名和导入关系，不改变策略行为。

其中 `submissions/robust_final_legacy_agent.py` 保存 2026-07-14 04:51 提交 `0c3c087` 的已验证策略，供最终主入口复用。`submissions/robust_new_agent.py` 是团队公共文件，保持当前 Git HEAD；除非团队明确决定合并，不再把个人实验直接写入该文件。
...
历史上对当时名为 `robust_new_agent.py`、后来曾命名为 `robust_cxy_agent.py`、现命名为 `robust_final_agent.py` 的曹潇月策略做过以下 Task 1 实测：
...
> 将 `robust_final_agent.py` 固定为曹潇月个人测试与最终提交主线，QN/DQN 只保留为实验对照；团队公共 `robust_new_agent.py` 单独维护。

### 4. 正式提交入口的任务绑定要求（已解决）

`robust_final_agent.py` 的 Task 4、Task 5 高层逻辑依赖 `task_id`。

如果只使用共享策略：

```powershell
--policy submissions/robust_final_agent.py
```
...
python utils/evaluate_policy.py `
  ...
  --task-policy mathematical_logic/task_1=submissions/robust_final_agent.py `
  --task-policy mathematical_logic/task_2=submissions/robust_final_agent.py `
  --task-policy mathematical_logic/task_3=submissions/robust_final_agent.py `
  --task-policy mathematical_logic/task_4=submissions/robust_final_agent.py `
  --task-policy mathematical_logic/task_5=submissions/robust_final_agent.py `
  --info-mode safe `
  --robustness-suite `
  --num-envs 100
```
...
- 使用五个 `--task-policy` 将同一个个人最终入口 `robust_final_agent.py` 绑定到五关；
...
- 固定 `submissions/robust_final_agent.py` 为曹潇月个人开发、测试和最终提交主线；
- `submissions/robust_new_agent.py` 保持团队上游 HEAD，不再承载个人未提交实验；
- `submissions/robust_final_legacy_agent.py` 保存 `0c3c087` 的已验证旧策略，仅作为最终版内部兼容实现；
- QN/DQN 只保留为实验对照；
...
个人最终命令应让五个 `--task-policy` 均显式指向同一个 `robust_final_agent.py`。公共 `check_final_agent_contract.py`、`check_task45_p0.py` 和 `evaluate_final_agent.ps1` 保持 HEAD，仍测试团队 `robust_new_agent.py`；不要为了个人测试修改它们。QN/DQN 不会被个人候选入口导入，因此其 monkey patch 不会影响 cxy 测评。

此前用等价的 cxy 显式绑定完成过契约检查，确认：

- 五关绑定到同一个 `robust_final_agent.Policy` 对象；
...
2026-07-19 实测通过：五关均绑定同一个 `robust_final_agent.py` 对象；`safe_info` 仅含 `last_reward`、`inventory` 和 `task_id`；五关均输出 `0..6` 合法动作；主策略、legacy、团队快照和 Task 5 分派状态均可由无参数 `reset()` 清空。公共 `check_final_agent_contract.py`、`check_task45_p0.py` 和 `evaluate_final_agent.ps1` 仍测试团队 `robust_new_agent.py`，不作为个人 cxy 证据。

实现上，个人最终主入口是 `submissions/robust_final_agent.py`：Task 4 通过 `robust_final_legacy_agent.py` 复用曹潇月提交 `0c3c087` 中已经验证的像素策略；Task 5 还会复用固定的 `robust_final_snapshot_agent.py`。默认渲染根据首房间的相对视觉几何选择通用探索策略或 legacy 战斗策略，不再使用 `(2, 2)` 等绝对坐标或 evaluator 的 `spatial_c` 标签。当前相对条件是“多出口 hub 中宝箱位于按钮左侧且怪物位于 NPC 下方”，整体平移和保持相对关系的小幅移动不会破坏分派。Task 5 两条危险开箱路线也不再固定第 1 行、第 4 行或底行，而是从可见入口、目标宝箱纵向位置和地图边界推导入口行及绕行方向。2026-07-19 重跑 Task 5 default、spatial A/B/C，分别以 1138、1148、1185、1130 步达到 `world_completed`；A/B/C 与修改前步数一致。该实现仍是双策略分派和相对几何路线，不声称对任意拓扑变化均已证明泛化。公共 `robust_new_agent.py` 不参与这些个人 P0 结果。
...
当前状态：**已完成**。当前重命名前的同一三文件策略已完成五关每关 100 episode 的 60/30/10 正式测评，完整结果为 492/500（98.4%），保存在 `eval_results/cxy_final_100.json`。唯一系统性失败是 Task 1/3/4/5 的 high_contrast；Task 2 的 high_contrast 通过。
...
```powershell
python utils/evaluate_policy.py --tasks mathematical_logic/task_1 mathematical_logic/task_2 mathematical_logic/task_3 mathematical_logic/task_4 mathematical_logic/task_5 --task-policy mathematical_logic/task_1=submissions/robust_final_agent.py --task-policy mathematical_logic/task_2=submissions/robust_final_agent.py --task-policy mathematical_logic/task_3=submissions/robust_final_agent.py --task-policy mathematical_logic/task_4=submissions/robust_final_agent.py --task-policy mathematical_logic/task_5=submissions/robust_final_agent.py --info-mode safe --robustness-suite --num-envs 10 --json-out eval_results/cxy_final_10.json
```
...
```powershell
python utils/evaluate_policy.py --tasks mathematical_logic/task_1 mathematical_logic/task_2 mathematical_logic/task_3 mathematical_logic/task_4 mathematical_logic/task_5 --task-policy mathematical_logic/task_1=submissions/robust_final_agent.py --task-policy mathematical_logic/task_2=submissions/robust_final_agent.py --task-policy mathematical_logic/task_3=submissions/robust_final_agent.py --task-policy mathematical_logic/task_4=submissions/robust_final_agent.py --task-policy mathematical_logic/task_5=submissions/robust_final_agent.py --info-mode safe --robustness-suite --num-envs 100 --json-out eval_results/cxy_final_100.json
```
...
当前状态：**进行中**。当前 cxy 的完整正式结果已固定为 `eval_results/cxy_final_100.json`（492/500，98.4%，对应重命名前的同一三文件策略）；仓库中已有未跟踪的 `report.tex` 草稿，但按当前约定不纳入本次代码提交。游戏/测评截图、报告内容核对、干净环境复现和候选提交 ZIP 尚未完成。
...
- [x] 唯一、明确的个人最终主入口：`submissions/robust_final_agent.py`，并配套两个固定辅助文件；
...
- [x] 报告中的成功率与 JSON、代码版本一致，并区分正式测评版本与后续定向回归；
- [x] 已说明训练或调试阶段使用完整 `info` 的范围；
- [x] 当前最终策略为规则与搜索策略，不需要模型权重；
- [ ] 依赖和运行路径已在干净环境中复核；
- [ ] 游戏截图和正式测评截图齐全；
...
1. 固定个人最终入口 `robust_final_agent.py`，保持团队公共 `robust_new_agent.py` 不动；
2. 正确绑定五个任务的 `task_id`；
3. P0、小样本、500 局正式评测和 Lean 关键机械连接均已完成；提交当前三文件规范重命名，形成可引用代码版本；
4. 用最终 JSON 核对 `report.tex` 中的成功率、步数、reward、milestone、定理列表和失败案例；
5. 补齐游戏与正式测评截图，并明确调试阶段完整 `info` 的使用边界；
6. 制作候选 ZIP，在干净环境中按五个 `--task-policy` 入口完成一次端到端复现；
7. 团队按同一评测口径确认是否采用该三文件版本作为最终提交。
